...
export default function OccasionalExpenses() {
  const navigate = useNavigate();
  const { occasional, occasionalTotal, occasionalCount, occasionalPage, loading, error, fetchOccasional, createOccasional, updateOccasional, removeOccasional } = useExpenseStore();
  const { categories, fetch: fetchCategories } = useCategoryStore();
  const addToast = useToastStore((s) => s.add);

  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<OccasionalExpense | null>(null);
  const [deleteItem, setDeleteItem] = useState<OccasionalExpense | null>(null);
  ...
  const enrichedData = occasional.map((item) => ({
    ...item,
  ...
  const openEdit = (item: OccasionalExpense) => {
    setEditItem(item);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (data: Parameters<typeof createOccasional>[0]) => {
    if (editItem) {
      ...
      addToast('success', 'Despesa avulsa criada com sucesso!');
    }
    setModalOpen(false);
    setEditItem(null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast('success', 'Despesa avulsa excluída com sucesso!');
      setDeleteItem(null);
    } catch (err) {
      addToast('error', (err as Error).message || 'Erro ao excluir despesa avulsa.');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-2xl font-bold text-slate-900">Despesas Avulsas</h1>
      ...
      {!loading && !error && occasional.length === 0 && (
        <EmptyState
          ...
          description="Clique em '+ Novo' para adicionar sua primeira despesa."
      ...
      {!loading && !error && occasional.length > 0 && (
        <>
          <DataTable
            ...
            onClick={(row) => navigate(`/expenses/occasional/${row.id}`)}
            onEdit={(row) => openEdit(row as unknown as OccasionalExpense)}
            onDelete={(row) => setDeleteItem(row as unknown as OccasionalExpense)}
          />
          <Pagination page={page} limit={20} total={occasionalCount} onPageChange={setPage} />
        </>
      )}

      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditItem(null); }} title={editItem ? 'Editar despesa avulsa' : 'Nova despesa avulsa'}>
